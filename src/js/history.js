  ...
  list.innerHTML = filtered.map(s => {
    ...
    const activityHtml = isCardio
      ? `<div class="card-meta" style="margin-bottom:6px">
          ...
          ${s.speed?.avg ? `<span>Avg ${s.speed.avg} ${s.type === 'Running' ? '/km' : s.type === 'Swimming' ? 'min/100m' : 'km/h'}</span>` : ''}
          ${s.speed?.max ? `<span>Max ${s.speed.max} ${s.type === 'Running' ? '/km' : s.type === 'Swimming' ? 'min/100m' : 'km/h'}</span>` : ''}
        </div>`
