...
A training tracker with an AI coaching layer. The deployed app is a single `index.html` (HTML + CSS + vanilla JS), assembled by `build.py` from modular source files in `src/`. Auth and API proxying is handled by `worker.js` (Cloudflare Worker). Data is stored per-user under `users/{User}/log.json` and synced via the Worker. There is no frontend framework, no package manager, and no build toolchain beyond `build.py`.

## Development workflow

Edit files in `src/`, run `build.py`, then push:
```bash
python build.py
git add src/ build.py index.html && git commit -m "..." && git push
```
...
| File | Purpose |
|---|---|
| `src/shell.html` | HTML structure; contains `{{STYLES}}` and `{{SCRIPTS}}` markers |
| `src/style.css` | All CSS |
| `src/js/state.js` | Global constants and shared mutable state (`sessions`, `lookup`, …) |
| `src/js/auth.js` | Login/logout, JWT token handling, all Worker sync functions |
| `src/js/exercises.js` | Exercise card UI, autocomplete, `getExercises()` |
| `src/js/log.js` | `saveSession()`, `resetForm()`, `onTypeChange()` |
...
| File | Purpose |
|---|---|
| `index.html` | Assembled app (do not edit directly — generated by `build.py`) |
| `build.py` | Assembles `index.html` from `src/` sources |
